...
import { Button, Modal, List, Link, Space, Message, Popconfirm, Input, Drawer } from '@arco-design/web-react';
import { IconSave, IconImport, IconFire, IconAlignCenter, IconRefresh, IconClockCircle, IconDown, IconShrink } from '@arco-design/web-react/icon';
...
class App extends React.Component {
    ...
    render() {
        ...
        return <div>
            <JSONEditor height={this.state.viewHeight} ref={this.onJSONEditorReady} json={this.state.value} onValidate={this.validateJSON} />
            <div style={{ textAlign: 'center', marginTop: '15px' }}>
                <Space>
                    <Button onClick={this.loadJSON} type='outline' icon={<IconImport />}>从文件加载</Button>
                    <Button onClick={this.saveJSON} type='outline' icon={<IconSave />}>保存到本地</Button>
                    <Button onClick={this.loadCacheJSON} type='outline' icon={<IconShrink />}>从缓存加载</Button>
                    <Popconfirm
                        title='请输入名字'
                        content={
                    ...
                    >
                        <Button type='outline' icon={<IconClockCircle />}>保存到缓存</Button>
                    </Popconfirm>
                    <Button onClick={this.clearJSON} type='outline' icon={<IconRefresh />}>清空输入</Button>
            ...
            >
                <List
                    ...
                    size='small'
                    render={(item, index) => <List.Item key={index} extra={
                        <Space size='mini'>
                            <Button onClick={this.importCacheJSON.bind(this, item)} type='text' size='mini'>导入</Button>
                            <Button onClick={this.copyCacheJSON.bind(this, item)} type='text' size='mini'>复制</Button>
                        </Space>
                    }>{item}</List.Item>}
                />
            </Drawer>
